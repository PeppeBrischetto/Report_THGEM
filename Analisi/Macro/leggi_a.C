...
void leggi_a(char *filename)
{
   ...
   char buffer[100];
   char title[100];
   //sprintf(title, "current channel %i", channelref);
   ...
   for(l=0; l<lines; l++){
   //while(!in.eof()){
      // salta i caratteri fino al valore corrispondente al canale scritto  
      cout<<"\n******** "<<l<<" **********"<<endl;
...
/// !!!!!!!! 
      ...
      // leggi il valore del canale
      in>>channel;  
      cout<<channel<<endl; 
   
      // salta i caratteri fino al nome della variabile
      for(int i=0;i<7;i++)
      {
         in.get(ch);
         //cout<<ch;
      ...
      // salta i caratteri fino al valore della variabile
      for(int i=0;i<6;i++)
      {
         in.get(ch);
         //cout<<ch;
      ...
      if (strcmp(name,namerefch) == 0 && channel==0){
         ...
         gra0->SetPoint(j0, secTot, value*1000);
   ...
   gra0->GetYaxis()->SetTitle("current (nA)");
   ...
   gra1->GetYaxis()->SetTitle("current (nA)");
   ...
   gra4->GetYaxis()->SetTitle("current (nA)");
   ...
   gra5->GetYaxis()->SetTitle("current (nA)");
