void plotTable_myversion(char* filename)
{
   double V, a, b, c, d;
   char buffer[200];
...
*/      
  ...
  gr4->SetTitle("ch6");
   ...
   c2a->SetGrid();
   c2a->SetLogy();  
   TH2F *bga=new TH2F("bga","",1000,160,230,1000,1.,1000.);
   bga->SetStats(0);
  ...
  leg->Draw();
      ...
      tex = new TLatex(205.9026,1,"I_{beam}~160pA");
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(203.3238,3,"Target=0.972mg/cm^{2}");
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(205.6877,5,"P=20.7mbar");
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(206.6547,.7,"V_{ind}=50V");
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(206.1175,.9,"V_{drift}=800V");
   tex->SetLineWidth(2);
   tex->Draw();
