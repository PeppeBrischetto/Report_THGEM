...
\begin{table} [b!]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{The values of pressure, \Vthgem{} and \Vdrift{} set for the study on \Vind.} \label{tab:FULLTHGEM_vind}
\end{table}
The \Vind{} scans have been carried out starting from 0~V and increasing the voltage by 10 or 20~V per run, until the discharge value is reached.
The result of the scan at 30~mbar and with \Vthgem{}~=~220~V is shown in Figure~\ref{fig:induction_FULLTHGEM_30mbar}: as expected, increasing \Vind{} the anodic current (blue line) goes up, while the top3 current (yellow line) approaches to zero.
This means that the stronger is the electric field in the induction region, the greater is the fraction of electrons collected at the anode. 
