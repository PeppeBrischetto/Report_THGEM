...
%Up to 140~V, when \Vind{} increases, the anodic current (blue line) goes up, the top3 current (yellow 
%line) approaches to zero, and their sum (orange line) is constant.
...
while the \Vthgem{} was ranging from 0 to a maximum value, corresponding to the discharge,at step of 5/10 V. 
...
Plotting the MF as a function of the reduced electric field\footnote{Some clarification on how
the reduced electric field was calculated. Obviously make no sense to speak about a value of the
electric field inside the THGEM since the electric field is not uniform there. We make a very
strong approximation cthat is calculate the electric field as it was uniform that is V/d} do 
not help Fig. \ref{}
 \ref{fig:multiplication_factor_FULL_F}
\begin{figure}[!t]
	\centering
	\includegraphics[width=\textwidth]{Immagini/MF_FULL_THGEM_noBeam_F.pdf}
	\caption{The multiplication factors evaluated for the FULL THGEM as a function of reduced
	eletric field.}
	\label{fig:multiplication_factor_FULL_F}
\end{figure}
...
%\begin{figure}[htbp]
%	\centering
%	\includegraphics[width=\textwidth]{Immagini/PosNeg_ROW_r124-138.pdf}
%	\caption{The TNC and TPC are compared shown in .}
%	\label{fig:ion_backflow_FULL}
%\end{figure}
...
The explored range goes from 0~V\footnote{or 100~V in the case with P~=~20~mbar} to the discharge voltage,
%which depends on the gas pressure: for P~=~30~mbar it was 1400~V, for P~=~20~mbar it was 1000~V, and for P~=~11~mbar it was 800~V.
%The increments were of 100~V for all the cases, but for P~=~11~mbar the first 400~V were scanned in step of 30~V.
A scheme of the different configuration used for the tes t is shown in Tab. \ref{tab:ROWTHGEM_vdrift}
\begin{table} [!h]
	\begin{center}
		\renewcommand{\arraystretch}{1.2}
		\begin{tabular} {ccccccc}
			P (mbar) & & \Vind{} (V) & & \Vthgem{} (V)& & \Vdrift\\
			\toprule[0.1em]
			%\hline
			30	& &	70	& &	240 & & 0-1400\\
			30	& &	70	& & 230 & & 0-1400\\
...
%
%\textcolor{red}{RIMUOVERE questo paragrafo. Aggiungere la nuova curva di figura 
%\ref{fig:thgem_FULLTHGEM_10mbar} in figura \ref{fig:multiplication_factor_FULLandROW}}
%
...
\subsubsection{Scan on \Vdrift}
\textcolor{red}{Rimuovere}
The currents measured during \Vdrift{} scan have the same behavior (almost constant) of previous 
test. The difference is the current values, a clear example can be anodic current: ~0.5 nA fixing 
\Vind{} = 50 V and \Vthgem{} = 160 V and ~11 nA fixing \Vind{} = 70 V and \Vthgem{} = 190 V.
...
Comparing this scan with a previous one done in similar conditions (P~=~11~mbar, \Vind{} = 70 V, 
\Vdrift{} = 600 V), which is shown in Figure~\ref{fig:drift_FULLTHGEM_11mbar}, the measured 
currents exhibit the same behaviour (almost constant).
...
\clearpage
\subsubsection{Test in asymm configuration}
This section is dedicated to some test performed with an asymmetric configuration of the THGEM bias.


A first test have been done using the THGEM ROW, keeping fix the value of the THGEM 1 and 2 (i.e. 
those closest to chatode) at 195 Volt each one. Increasing just the V THGEM3 the change in the 
gain of the MTHGEM is exponential as usual

\begin{figure}[htbp]
	\centering
    \includegraphics[width=\textwidth]{Immagini/gainVSVTHGEM_Asym.pdf}
	\caption{Gain as a function of the \Vthgem{}3.}
	\label{fig:THGEMScanAsym}
\end{figure}

In fig. \ref{fig:IBF_THGEMROW_asym_10mbar} the IBF using an asymmetric configuration is compared 
with the IBF obtained using a symmetric configuration.
In order to compare two different confgurations they should have the same gain.
In other words to understand if an asymmetric configuration has a lower IBF than a symmetric configuration
we should compare two configurations with same gain since we know that the IBF depends on the gain.
In practice it is very difficult and time consuming to look for an asymmetric configuration with
the same gain of a simmetric one.
In order to avoid this, we decide to compare the IBF data of a whole THGEM scan (orange points in
fig. \ref{fig:IBF_THGEMROW_asym_10mbar}) with those of an asymmetric scan in the plot
IBF vs Gain Fig. \ref{fig:IBF_THGEMROW_asym_10mbar}. 

From the plot is evident that there is no difference between a symmetric and an asymmetric
configuration using the ROW THGEM. This is true both at high \Vdrift V=700 and at low \Vdrift V=200.

Two asymmetric configurations have been tested at \Vdrift=200. In the first configuration
the \Vthgem$_3$ was V=195 while the value of the other two THEGEM were decresed.
In the second configuration \Vthgem$_1$ was V=195 while the other two were slowly decreased.
No difference was observed in the symmetric and in the two asymmetric configuration.
\begin{figure}[htbp]
	\centering
    \includegraphics[width=\textwidth]{Immagini/THGEM_ROW_IBFwithAsymmConfig.pdf}
	\caption{IBF as a function of the gain for asymmetric and symmetric configurations.}
	\label{fig:IBF_THGEMROW_asym_10mbar}
\end{figure}


Same test have been performed with FULL THEGEM. In this case seems that a small effect on the 
IBF appears for the lower values of the gain.
We observed that in this case the (elettrodi intermedi) measure a positive current that is not
neglegible. \textcolor{red}{E necessaria una indagine più approfondita e sistematica
sulle correnti lette dagli elettrodi intermedi che sono stati fino ad ora (colpevolmente) 
ignorati.}

\begin{figure}[htbp]
	\centering
    \includegraphics[width=\textwidth]{Immagini/THGEM_THGEM10_IBFwithAsymmConfig.pdf}
	\caption{IBF as a function of the gain for asymmetric and symmetric configurations.}
	\label{fig:IBF_THGEM10_asym_10mbar}
\end{figure}
...
%Another important difference is the maximum achievable anodic current: $\sim -$20 nA with 
%$\alpha$ source, $\sim$120 nA with \ibeam = 60 pA and $\sim -$280 nA with \ibeam = 400 pA.
...
(TBFC1) and a second downstram the target (TBFC2) that has a lower sensitivity.
...
\subsection{FULL THGEM}
  ...
  The two scans of the \Vthgem{} done at low rate xx and high rate xx are shown in Fig.
  \ref{fig:driftScan_withBeam_H}.
  \begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Immagini/thgemScan_THGEM10_20mbar-Vdrift1000V-2020-03-09.pdf}
...
%  \end{figure}
  ...
  In Fig. \ref{fig:MF_FULL_beam} a comparison of the multiplication factor obtained with
  $\alpha$ source at different pressures and thise for the beam at 20 mbar. The MF curves for 
  the beam do not correspond to those for $\alpha$ at the same gas pressure.
  ...
  \begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Immagini/IBF_FULL_Comparison_F.pdf}
	\caption{IBF for the FULL THGEM as a function of the reduced electric field. Run with 
...
%	\label{fig:driftScan_withbeam_L}
 % \end{figure}
  ...
  In Fig. \ref{fig:MF_ROW_beam} a comparison of the multiplication factor obtained with
  $\alpha$ source at different pressures and thise for the beam at 20 mbar. The MF curves for 
  the beam perfectly superimpose those for $\alpha$ at the same gas pressure.
  ...
  \subsubsection{Drift scan}
  We compare the \Vdrift{} scan get with $\alpha$-source and with beam.
  If we plot them as a function of the reduced electric field Fig. \ref{fig:driftScan_withbeam_F} 
  we see that the behaviour is the same for all the run. 
  %If we plot the same data as a function  of the voltage \Vdrift{} the behaviour 
  %\ref{fig:IbeamScan_THGEM10_10mbar_average}
  \begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Immagini/driftScan_ROW_THGEM_comp.pdf}
  ...
  \begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Immagini/driftScan_ROW_THGEM_comp_F.pdf}
	\caption{Current of the anode as a function of the reduced electrc field E/p for the 
	ROW THGEM.}
	\label{fig:fig:driftScan_withbeam_F}
  \end{figure}
