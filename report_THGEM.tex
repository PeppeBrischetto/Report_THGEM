...
This value is dependent on the gas pressure: for P~=~30~mbar it was 220~V, for P~=~20~mbar it was 200~V, and for P~=~11~mbar it was 150~V.
...
The results for P~=~20~mbar and P~=~11~mbar are respectively shown in Figure~\ref{fig:thgem_FULLTHGEM_30mbar_other_pressure_a} and~\ref{fig:thgem_FULLTHGEM_30mbar_other_pressure_b}.
...
\begin{figure}[!htb]
	\centering
	\subfigure[]{ \label{fig:thgem_FULLTHGEM_30mbar_other_pressure_a} \includegraphics[width=0.96\textwidth]{Immagini/thgemScan_THGEM10_20mbar.pdf}}
	\subfigure[]{ 	\label{fig:thgem_FULLTHGEM_30mbar_other_pressure_b} \includegraphics[width=0.96\textwidth]{Immagini/thgemScan_THGEM10_10mbar.pdf}}
	\caption{Currents measured during the scan on the voltage \Vthgem{} across each THGEM: in (a) at 20~mbar, in (b) at 11~mbar.}
	\label{fig:thgem_FULLTHGEM_30mbar_other_pressure}
\end{figure}
...
The explored range goes from 0~V (or 100~V in the case with P~=~20~mbar) to the discharge voltage, which depends on the gas pressure: for P~=~30~mbar it was 1500~V, for P~=~20~mbar it was 1200~V, and for P~=~11~mbar it was 800~V.
...
All the measured currents are saturated already at 100~V.

At \Vdrift~=~5~V the anodic current is saturated. 
This means that the magnitude of the electric field at \Vdrift~=~0~V is much lower than $5/20\; \mbox{(V/cm)}= 250 \; \mbox{(mV/cm)}$ or the value measured by PICO is actually not zero.
