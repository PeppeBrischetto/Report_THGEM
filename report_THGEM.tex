...
%The explored range of \Vthgem{} was determined in this way: the lower extreme is the minimum value at which a variation of the measured currents is visible, while the higher extreme is the discharge value.
...
The range depends on the selected pressure and on \Vind{} and \Vdrift. 
...
The chosen values of pressure, \Vind{} and \Vdrift{} are shown in Table~\ref{tab:FULLTHGEM_vthgem}, together with the corresponding \Vthgem{} explored ranges.
...
\begin{table} [!h]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{The values of pressure, \Vind{} and \Vdrift{} adopted for the study on \Vthgem{} and the explored \Vthgem{} range.} \label{tab:FULLTHGEM_vthgem}
\end{table}
...
The first test with beam was done on February 19 2020. We tested FULL THGEM \textit{F10} at 60 pA, 400 pA, 1 nA and 1.8 nA of beam current (\ibeam). The pressure was 9.4 mbar.

\subsubsection{Scan on \Vthgem}

The measurement was done fixing \Vind{} = 50~V and \Vdrift{} = 400~V. 
The results are shown in Figure~\ref{fig:thgemScan_THGEM10_beam_10mbar}.
The currents have the same behaviour of the scan on \Vthgem{} with $\alpha$ source (see Figure~\ref{fig:thgem_FULLTHGEM_10mbar_bis}). The discharge was at \Vthgem{} = 190 V with beam, instead of 210~V with $\alpha$ source. Another important difference is the maximum achievable anodic current: $\sim -$20 nA with $\alpha$ source, $\sim$120 nA with \ibeam = 60 pA and $\sim -$280 nA with \ibeam = 400 pA.
...
The results are shown in Figure~\ref{fig:driftScan_THGEM10_beam_10mbar}.
The currents have the same behaviour of the scan on \Vdrift{} with $\alpha$ source (see Figure~\ref{fig:driftScan_THGEM10_alpha_10mbar}). The discharge was at \Vdrift{} = 800 V with beam, instead of more than 700~V with $\alpha$ source. Another important difference is the maximum anodic achievable current: $\sim - $0.5 nA with $\alpha$ source, $\sim - $20~nA with \ibeam = 60~pA and $\sim - $55~nA with \ibeam = 400 pA.
...
Figure~\ref{fig:IBFvsDrift_beam_alpha_average} shows a comparison between the IBFs with $\alpha$ source and those with beam.
The IBF for 60 pA, 400 pA at \Vthgem{} = 160 V and $\alpha$ source at \Vthgem{} = 190 V has a maximum IBF of 17\%. 
In the test with $\alpha$ source, when \Vthgem{} is less than 190 V the cathode current is so small that IBF estimate is not reliable. IBF as a function of \Vdrift{} seems not sensible to the rate because it has a constant rise with $\alpha$ source and with beam. 
%This is not well understood.
Looking at the circle points, it seems that increasing the beam current and fixing all the other parameters, the IBF decreases. 
...
\subsubsection{Scan on the rate of particles}
...
Since to increase the beam intensity up to 1 nA and 1.8 nA a collimator was removed from the beam line, the beam spot and its distribution are different respect to the measures at 60 and 400~pA.
This means that the last two points are not in the same conditions of the first two, so that they actually cannot be compared.
Nonetheless, we found that the currents increase with increasing \ibeam.
